...
def generate_response(messages, temperature=0.5, n=1, max_tokens=4000, frequency_penalty=0):
...
def build_prompt(genre, target_audience, theme, setting, content_hint, narrator="omniscient", twist=False):
    
    if twist:
        prompt = f"Write a {genre} children's book targeted at {target_audience} about {theme}.  It takes place in {setting}. Use a {narrator} narrator. Include a surprise twist at some point in the story."
    
    else:
        prompt = f"Write a {genre} children's book targeted at {target_audience} about {theme}.  It takes place in {setting}. Use a {narrator} narrator. {content_hint}"
...
def select_best_outline(enhanced_prompt):
    ...
    outlines_completion = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system", "content": "You are a helpful assistant."},
            {"role": "user", "content": f"You are an incredibly talented children's book author.  You know how to weave a story together in a way that is engaging and fun to read.  You keep things fresh, exciting, and ensure that your stories aren't repetitive.\nCreate an outline for a children's book based on this prompt: '{enhanced_prompt}'"}
        ],
        n=5,
        temperature=1.2,
    )
...
def build_first_draft(outline):
    ...
    for prompt in section_prompts[1:]:
        ...
        messages.append({"role": "user", "content": f"Write a section for a children's book based on this part of the outline, ensuring that the wording is not repetitive: '{prompt}'"})
        section_content = generate_response(messages, temperature=0.7, max_tokens=2000, frequency_penalty=0.8)
...
def check_and_combine_sections(prev_section, cur_section):
    messages = [
        {"role": "system", "content": "You are a helpful assistant that combines repetitive sections."},
        {"role": "user", "content": "You are a very talented editor, who takes two sequential sections of a book and check to ensure they are not redundant, and that they're both necessary to the plot of the story. If they aren't, you combine them into one section that is not redundant, is concise, yet maintains the tone, characters and feeling of the section."},
        {"role": "assistant", "content": "Of course, what sections can I help you with today?"},
        {"role": "user", "content": f"Previous section: {prev_section}"},
        {"role": "user", "content": f"Current section: {cur_section}"},
        {"role": "user", "content": "Are these sections repetitive? If so, make the first word of your response 'Yesindeedy', and then combine them into a single unique section."}
    ]
...
def refine_draft(book_sections, title):
    ...
    messages = [
        {"role": "system", "content": "You are a helpful assistant."},
        {"role": "user", "content": "You are an extremely talented editor of children's books.  Your specialty is ensuring that the story maintains cohesion, continuity and flows well together, while not being redundant or repetitive.  We're going to ensure that this children's story maintains continuity, step by step."},
        {"role": "user", "content": "This is the first section: {refined_sections[0]}."}
    ]
    ...
    for i in range(1, len(book_sections)):
        ...
        messages.append({"role": "user", "content": continuity_check_prompt(curr_section)})
        response = generate_response(messages, temperature=0.2, max_tokens=5)

        while response.lower() not in ('true', 'false', 'true.', 'false.'):
            messages.append({"role": "user", "content": "Please respond with only one word, 'true' or 'false'."})
            response = generate_response(messages, temperature=0.2, max_tokens=5)
            print(response)
    ...
    messages.append({"role": "user", "content": continuity_check_prompt_first(first_section, last_section)})
    response = generate_response(messages, temperature=0.2, max_tokens=5)
...
def generate_photo(outline, title):
    ...
    messages = [
        {"role": "system", "content": "You are a helpful assistant."},
        {"role": "user", "content": "You are my Photo Prompt Creator.  You take book outlines, and create a one sentence prompt to be used by openai.Image.create().  The prompt will describe a picture of a scene from the book, in a cartoon/animated style.  A cartoon picture you might use on the cover of the children's book.  Have it match the tone and feeling of the outline."},
        {"role": "assistant", "content": "Great! Please provide me with the book outline and any specific instructions or preferences you have for the tone and feeling of the prompt."},
...
def create_finished_copy(image_path, title, story_text, output_path):
    ...
    title_style = ParagraphStyle(name='TitleStyle', parent=styles['Title'], fontSize=24, alignment=TA_CENTER)
    ...
    # Add the story
    story_style = ParagraphStyle(name='StoryStyle', parent=styles['Normal'], fontSize=14)
